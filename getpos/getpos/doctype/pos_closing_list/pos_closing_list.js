...
//                         closing_amount: totalAmountGlobal,
//                         difference: flt(opening_amount - closing_amount),
//                     });
//                 });
...
//                         });
//                          totalAmountGlobal = total_amount;
//                     }
//                     frm.set_value({
            ...
            .then(({ balance_details }) => {
                balance_details.forEach(detail => {
                    frm.add_child("payment_reconciliation", {
                        mode_of_payment: detail.mode_of_payment,
                        opening_amount: detail.opening_amount,
                        expected_amount: totalAmountGlobal,
                        closing_amount: totalAmountGlobal, // Set closing_amount using totalAmountGlobal
                        difference: flt(detail.opening_amount - totalAmountGlobal), // Calculate difference based on opening_amount and totalAmountGlobal
                    });
                });
                frm.refresh_field("payment_reconciliation");
            })
        ...
        return frappe.call({
            ...
            callback: function(response) {
                if (response.message && response.message.length > 0) {
                    ...
                    totalAmountGlobal = total_amount; // Update totalAmountGlobal here
                    frm.set_value({
                        grand_total: flt(total_amount),
                        net_total: flt(net_total),
                        total_quantity: flt(quantity),
